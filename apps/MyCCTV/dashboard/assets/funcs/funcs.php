<?php
	...
	function deviceExiste($name)
	{
		global $mysqli;

		$stmt = $mysqli->prepare("SELECT id_device FROM devices WHERE name = ? LIMIT 1");
	...
	function registraDevice($name, $ip, $port, $user, $password, $id_usuario){

		global $mysqli;

		$stmt = $mysqli->prepare("INSERT INTO devices (name, ip, port, user, password, id_usuario) VALUES(?,?,?,?,?,?)");
		$stmt->bind_param('ssissi', $name, $ip, $port, $user, $password, $id_usuario);
	...
	function enviarEmail($email, $nombre, $asunto, $cuerpo){
		...
		$mail->setFrom('tecnicoacr@gmail.com', 'Cuentas de Usuario');
...
function enviarEmail2($email, $nombre, $asunto, $cuerpo){
		...
		if($rows > 0) {
			...
			if(isActivo($usuario)){
				...
				if($validaPassw){
					...
					$errors = "La contrase&ntilde;a o el usuario son incorrectos";
				}
				} else {
	...
	function lastSession($id_usuario)
	{
		global $mysqli;
	...
	function getPasswordRequest($id_usuario)
	{
		global $mysqli;
